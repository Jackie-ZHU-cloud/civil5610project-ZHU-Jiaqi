...
"""
Scheme 2: Congestion Pricing
- Charge 10 HK$ on selected congested links
- 10 HK$ = 0.5 minutes (VOT=20 HK$/min)
- Solve UE again
"""
...
print("=== Scheme 2: Congestion Pricing ===")
...
for i in range(10):
    a=idx[i]+1
    print(f"{i+1}. Link {a}: {tail[a]}->{head[a]}, flow={flow_ue[a]:.2f}, v/c={vc[a-1]:.2f}")


import numpy as np
np.save("flow_scheme2.npy", flow_ue)       # scheme 2 UE
